...
onMounted(async () => {
  ...
  try {
    const response = await axios.get(
      "https://2ee9-103-100-175-121.ngrok-free.app/api/categories",
      {
        headers: {
  ...
  if (storyIdFromRoute) {
    ...
    try {
      const response = await axios.get(
        `https://2ee9-103-100-175-121.ngrok-free.app/api/stories/${storyIdFromRoute}`,
        {
          headers: {
            "ngrok-skip-browser-warning": "69420",
            Authorization: `Bearer ${authStore.token}`,
      ...
      images.value = story.images
        ? story.images.map((image) => ({
            url: image.url
              ? `https://2ee9-103-100-175-121.ngrok-free.app${image.url}`
              : "",
            id: image.id,
          }))
        : [];
...
const uploadStory = async () => {
  ...
  try {
    const url = isEditMode.value
      ? `https://2ee9-103-100-175-121.ngrok-free.app/api/stories/${storyId.value}`
      : "https://2ee9-103-100-175-121.ngrok-free.app/api/stories";
